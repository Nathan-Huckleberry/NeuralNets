...
public class MultiLevelStructWithBias extends Structure {
	public MultiLevelStructWithBias(Node.Type t, int... numNodes)
	{
		...
		for(int i = 0; i < numNodes.length; i++)
		{
			...
			if(i!=numNodes.length-1)
				thisLayer.add(new Node(Node.Preset.CONSTANT));
			if(i==numNodes.length-1)
	...
	public MultiLevelStructWithBias(Node.Preset p, int... numNodes)
	{
		if(numNodes.length<2)
			throw new IllegalArgumentException("Not enough layers for input and output layers, layers must be >= 2");
		ArrayList<Node> input = new ArrayList<>();
	...
	public MultiLevelStructWithBias(ArrayList<Node> in, ArrayList<Node> out) {
		super(in, out);
